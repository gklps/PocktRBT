...
import type { NFT, FT } from '../types';

const Assets = () => {
  const { token, user } = useAuth();
  const { accentColor } = useTheme();
  const [activeTab, setActiveTab] = useState<'NFT' | 'FT' | 'Smart Contracts'>('NFT');
  ...
  const handleTransferFT = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token || !user?.did || !selectedFT) return;
    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="flex space-x-2 mb-6">
          {(['NFT', 'FT', 'Smart Contracts'] as const).map((tab) => (
            <button
              key={tab}
            ...
            {loading ? (
              ...
              <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
                {nfts.map((nft) => (
                  <div
                    key={nft.nft}
                    className={`p-4 border border-${accentColor}-200 rounded-lg bg-${accentColor}-50`}
                  >
        ...
        {activeTab === 'FT' && (
          <div className="space-y-6">
            ...
            {loading ? (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg bg-${accentColor}-100 hover:bg-${accentColor}-200 transition-colors`}
                      >
              ...
              <p className="text-gray-500">No FTs found</p>
            )}
                ...
                <div className="bg-white rounded-lg p-6 max-w-md w-full">
                  ...
                  <form onSubmit={handleTransferFT} className="space-y-4">
                    <div>
                      ...
                      <input
                        type="text"
                        value={transferData.receiverDid}
                        onChange={(e) =>
                          setTransferData({ ...transferData, receiverDid: e.target.value })
                        }
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        min="1"
                        max={selectedFT.ft_count}
                        value={transferData.ftCount}
                        onChange={(e) =>
                          setTransferData({
                            ...transferData,
                    ...
                    <div className="flex space-x-2">
                      <button
                        type="submit"
                        className={`flex-1 py-2 bg-${accentColor}-600 text-white rounded-lg hover:bg-${accentColor}-700`}
                      >
                        Transfer
                      </button>
                      <button
                        type="button"
                        onClick={() => setSelectedFT(null)}
